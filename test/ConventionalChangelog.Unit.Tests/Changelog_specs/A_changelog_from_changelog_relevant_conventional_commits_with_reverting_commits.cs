...
public partial class A_changelog_from_changelog_relevant_conventional_commits
{
    public class With_reverting_commits
    {
        ...
        {
            _reverted = Feature.CommitWithDescription(1);
            _reverting = Feature.CommitWithDescription(2).WithFooter(DefaultRevertToken, _reverted.Hash);
        }
        ...
        public void when_a_reverted_reverting_commit_is_part_of_the_changelog_shows_the_originally_reverted_commit()
        {
            var revertingReverting =
                Feature.CommitWithDescription(3).WithFooter(DefaultRevertToken, _reverting.Hash);
        ...
        {
            var reverting2 = Feature.CommitWithDescription(3).WithFooter(DefaultRevertToken, _reverted.Hash);
        ...
        {
            var reverting = Feature.CommitWithDescription(2).WithFooter(footer, _reverted.Hash);
        ...
        [Theory]
        [CaseVariantData(DefaultRevertToken)]
        public void recognizes_reverting_multiple_commits(string footer)
        {
            var reverted2 = Feature.CommitWithDescription(2);
            var reverting = Feature.CommitWithDescription(3)
                .WithFooter(footer, _reverted.Hash)
                .WithFooter(footer, reverted2.Hash);
            var changelog = The.ChangelogFrom(reverting, reverted2, _reverted);
