...
public partial class A_changelog_from_changelog_relevant_conventional_commits
{
    public class With_overriding_commits
    {
        ...
        {
            _overridden = Feature.CommitWithDescription(1);
            _overriding = Feature.CommitWithDescription(2).WithFooter(DefaultOverrideToken, _overridden.Hash);
        }
        ...
        public void when_the_overridden_and_overriding_commit_are_part_of_the_changelog_shows_the_overriding_commit()
        {
            ...
            changelog.Should().Be(A.Changelog.WithGroup(Feature, 2));
        }
        ...
        {
            var overridingOverriding = Feature.CommitWithDescription(3).WithFooter(DefaultOverrideToken, _overriding.Hash);

            var changelog = The.ChangelogFrom(overridingOverriding, _overriding, _overridden);

            changelog.Should().Be(A.Changelog.WithGroup(Feature, 3));
        }
        ...
        {
            var overriding2 = Feature.CommitWithDescription(3).WithFooter(DefaultOverrideToken, _overridden.Hash);

            var changelog = The.ChangelogFrom(overriding2, _overriding, _overridden);

            changelog.Should().Be(A.Changelog.WithGroup(Feature, 3, 2));
        }
        ...
        {
            var overriding = Feature.CommitWithDescription(2).WithFooter(footer, _overridden.Hash);
            var changelog = The.ChangelogFrom(overriding, _overridden);
        ...
        {
            var overridingOverriding = Feature.CommitWithDescription(2).WithFooter(footer, _overridden.Hash);
            var changelog = The.ChangelogFrom(overridingOverriding, _overridden);

            changelog.Should().Be(A.Changelog.WithGroup(Feature, 2, 1));
        }

        [Theory]
        [CaseVariantData(DefaultOverrideToken)]
        public void recognizes_overriding_commit_with_multiple_targets(string footer)
        {
            var overridden2 = Feature.CommitWithDescription(2);
            var overridingOverriding = Feature.CommitWithDescription(3)
                .WithFooter(footer, _overridden.Hash)
                .WithFooter(footer, overridden2.Hash);
            var changelog = The.ChangelogFrom(overridingOverriding, overridden2, _overridden);
