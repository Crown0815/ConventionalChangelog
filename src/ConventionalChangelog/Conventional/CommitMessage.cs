...
{
    public static CommitMessage Parse(string rawMessage, ITypeFinder configuration) =>
        MessageParser.Parse(rawMessage, configuration);

    internal static CommitMessage Parse(Commit commit, ITypeFinder configuration) => Parse(commit.Message, configuration) with { Hash = commit.Hash };
